<?php
/* ============================================================================
 * LEFTPANE.php (Pipeway Left side )
 * ============================================================================
 * Author: KEANT Technologies              Date: 18 MAY 2026
 *
 * Purpose:
 * This module handles account search navigation and parameterized routing
 * for Account Detail and related child modules.
 *
 * Previous Behavior:
 * - Navigation relied on shared PHP SESSION variables
 * - Multiple browser tabs overwrote account context
 * - Child pages opened incorrect account details
 *
 * New Behavior:
 * - Account number passed through URL parameter (?acc=)
 * - Base64 encoded account context
 * - Independent tab-safe account navigation
 *
 * Key Changes:
 * - Added parameterized routing using ?acc=
 * - Updated window.open() and redirect URLs
 * - Removed dependency on shared session account context
 *
 * Change Tag:
 * ----------------------------------------------------------------------------
 * AH18052026 | Parameterized account routing implementation
 * ----------------------------------------------------------------------------
 * ============================================================================ */
       ...
       <?php /*for document transfer======================================================================   */?>
        ...
        <li class="treeview">
            ...
            <ul class="treeview-menu">
                ...
                <li>
                    <a href="/bi/dist/mydownload" id="<?php echo $mydownload; ?>"><img src="/img/file-download.png" alt="My Downloads" style="width: 13px; margin-right: 7px;">My Downloads</a>
                </li>
                  <?php }  if($fetchsubmenudoct['SUB_MENU_ITEM']=='MyDownloads'){ ?>
                <li><a href="/bi/dist/myupload" id="<?php echo $myupload; ?>"><img src="/img/file-upload.png" alt="My Uploads" style="width: 13px; margin-right: 7px;">My Uploads</a></li>
...
success: function(data) {
    ...
    if(data == '0') {
        swal(
            "Error!",
            "Your search returned no record. Please check your search and try again.",
            "error"
        );
    }
    else if(data == '1') {
        ...
        if(page == 'accountdetail') {            
            // window.location.href = "searchacc";
            var encodedAcc = btoa(accountno);
            window.location.href ="searchacc?acc=" + encodeURIComponent(encodedAcc);
        }
    else if(page == 'documentation') {
            // window.location.href = "plcmntjudgmntdocs";
           window.location.href = "plcmntjudgmntdocs?acc=" + encodeURIComponent(btoa(accountno));
        }
    else if(page == 'portfoliodocs') {
            // window.open("CLIENTGUIDE/ViewClientGuidesearchacc", '_blank');
            window.open("CLIENTGUIDE/ViewClientGuidesearchacc?acc=" + encodeURIComponent(btoa(accountno)), '_blank');
        }
    else if(page == 'settlementform') {
            // window.open("Settlement_Form/settlement-request", '_blank');
            window.open("Settlement_Form/settlement-request?acc=" + encodeURIComponent(btoa(accountno)), '_blank');
        }
    else if(page == 'statusupdate') {
            // window.open("StatusUpdate", '_blank');
            window.open("StatusUpdate?acc=" +encodeURIComponent(btoa(accountno)), '_blank');
        }
    }
        ...
        if(page == 'accountdetail') {
            // window.location.href = "searchacc";
            window.location.href = "searchacc?acc=" + encodeURIComponent(btoa(accountno));
        }
        else if(page == 'documentation') {
            // window.location.href = "plcmntjudgmntdocs";
           window.location.href ="plcmntjudgmntdocs?acc=" + encodeURIComponent(btoa(accountno)); 
