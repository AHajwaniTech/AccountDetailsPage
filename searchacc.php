...
session_start();
/* ============================================================================
 * ACCOUNT DETAIL MODULE (SEARCHACC)
 * ============================================================================
 * Author: KEANT Technologies              Date: 18 MAY 2026
 *
 * Purpose:
 * This module displays Account Detail information and controls routing
 * to all downstream account-related child modules.
 *
 * Previous Behavior:
 * - Account context retrieved from shared PHP SESSION
 * - Multiple browser tabs caused account overwrite issues
 * - Child pages displayed incorrect account information
 *
 * New Behavior:
 * - Account context retrieved using URL parameter (?acc=)
 * - Child pages receive parameterized account routing
 * - Tab-safe isolated account navigation implemented
 *
 * Key Changes:
 * - Replaced SESSION-based account retrieval with $_GET['acc']
 * - Updated child page hyperlinks and window.open() calls
 * - Removed shared account session dependency
 * - Removed legacy RMSFILENUM transformation logic
 *
 * Child Modules:
 * - FirmRecoveries.php
 * - ClientRecoveries.php
 * - FirmCost.php
 * - ClientCost.php
 * - StatusUpdate.php
 * - jugBreakdown.php
 ...
 * Change Tag:
 * ----------------------------------------------------------------------------
 * AH18052026 | Parameterized account handling for tab isolation
 * ----------------------------------------------------------------------------
 * ============================================================================ */
...
//AH18052026 : Start
$que1 = "UPPER(A.RMSACCTNUM) ='".$accountno."'";
...
$nameQuery="SELECT RGUSER FROM WSREGUSR WHERE RUTYPE='".$usertype."' and ".$code." and PWAY2MAIL='".$_SESSION['email']."'"; 
//echo $nameQuery;exit;
$resultname=mysqli_query($conn,$nameQuery);
$fetchname=mysqli_fetch_assoc($resultname);
//print_r($fetchname);exit;
               ...
               <div class="col-sm-12 col-xs-12">
                  ...
                  <div class="col-sm-6 col-xs-6 pl0">
                     ...
                     <div class="col-sm-6 col-xs-6 p0" id="firmdesc">
                        ...
                        <!-- //AH18052026 -->
                         <!-- <a href="PlcmntEmp" target="_blank"> -->                          
                          <a href="PlcmntEmp?acc=<?php echo urlencode($AccountNo); ?>" target="_blank">

                          <u><?php echo $row['Place_of_Employment'];?></u></a>
                         <?php } ?> </div>
                        <div class="col-sm-12 col-xs-12 p0"><?php if($row['Defendant_Attorney']=='YES'){?>
                           <a href="AttorneyInfo" target="_blank"><u>Defendant Attorney</u></a>
                           <?php }else {echo "--";}?></div>
            ...
            <div class="col-sm-12 col-xs-12">
               <div class="col-sm-6 col-xs-6 p0">
                  ...
                  <div class="col-sm-6 col-xs-6" id="currentstatusdesc">
                     ...
                     <!-- AH18052026 -->                     
                     <a href="StatusUpdate?acc=<?php echo urlencode($AccountNo); ?>" target="_blank"> Click Here</a>
                     <!-- Settlement Form URL -->  
                     <div class="col-sm-12 col-xs-12 p0">
                              <?php 
                              ...
                              if($check_code ==  $sif_code ){ ?>
                                ...
                                <a href="Settlement_Form/settlement-request" target="_blank">Click Here</a></br>
                              <?php  }
                                else{
                                  echo "<br>";
                                }
      ...
      <div class="col-sm-4 col-xs-4 pr0">
         ...
         <div class="col-sm-4 col-xs-4 pr0">
               ...
               <div class="col-sm-3 col-xs-3 pr0" id="judgementdesc">
                  ...
                  <!-- //AH18052026 -->                    
                      <a href="jugBreakdown?acc=<?php echo urlencode($AccountNo); ?>" target="_blank">

                      <u><?php echo $row['Judgment_Amount'];?></u></a>
                    <?php } ?> </div>
               </div>
        </div>
  ...
  $(document).ready(function(){
   ...
   if(id==3){
    ...
    window.open('/bi/dist/CLIENTGUIDE/ViewClientGuidesearchacc?acc=' + encodeURIComponent(accNo),'_blank');
     //window.open("https://quay.aacanet.org/AACANet/members/PortfolioSearch.aspx?number=<?php echo base64_encode($row['RMSFILENUM']);?>");
   }
   if(id==4){
    //AH18052026    
    window.open('Settlement_Form/settlement-request?acc=' + encodeURIComponent(accNo),'_blank');
    // window.open("Settlement_Form/settlement-request=<?php //echo $filenum;?>&username=<?php //echo $emailencode;?>");

     /*window.open("https://quay.aacanet.org/aacanet/members/SIFRequest/Settlement_Form.aspx?number=<?php //echo base64_encode($row['RMSFILENUM']);?>");*/
   }
  });
